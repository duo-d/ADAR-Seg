...
class Tree():
    ...
    def plot(self,label):
        ...
        coords = self.get_coordinates()
        plt.scatter(coords[:, 0], coords[:, 1], color='magenta', label = label, s=5)
        


    def plot_path(self, path, color='cyan', label='Solution Path',linewidth=3 ):
        ...
        for i in range(K-1):
            ...
            plt.plot(c[:, 0], c[:, 1], color=color, linewidth=linewidth)
        plt.scatter(path[:, 0], path[:, 1], color=color, label = label)
