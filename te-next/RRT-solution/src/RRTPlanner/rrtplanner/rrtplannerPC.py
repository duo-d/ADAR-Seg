...
class RRTPlannerPC():
    ...
    """
    def __init__(self, start, goal, pc_traversable, pc_obstacles, epsilon, max_nodes, robot_radius):
        ...
        self.pc_traversable = pc_traversable
        # a set of 3D points considered non-traversable (obstacles)
        self.pc_obstacles = pc_obstacles  
        ...
        points1 = self.build_inner_circles( inner_radius=0.2, outer_radius=0.4, z=0.0) + np.asarray(start) #AÑADO 
        points2 = self.build_inner_circles( 0.6, 0.8, 0.0) + np.asarray(start) #AÑADO 
        points3 = self.build_inner_circles( 1, 1.2, 0.0) + np.asarray(start) #AÑADO 
        points4 = self.build_inner_circles( 1.4, 1.6, 0.0) + np.asarray(start) #AÑADO 
        points5 = self.build_inner_circles( 1.8, 2, 0.0) + np.asarray(start) #AÑADO 
        points6 = self.build_inner_circles( 2.2, 2.4, 0.0) + np.asarray(start) #AÑADO 
        points7 = self.build_inner_circles( 2.6, 2.8, 0.0) + np.asarray(start) #AÑADO 
        points8 = self.build_inner_circles( 3, 3, 0.0) + np.asarray(start) #AÑADO 
        # points9 = self.build_inner_circles(3.4, 3.6, 0.0) + np.asarray(start) #AÑADO

        # self.pc_traversable = np.vstack(self.pc_traversable)
        self.pc_traversable = np.vstack((self.pc_traversable, points8, points7, points6, points5, points4, points3, points2, points1))
        # if self.collision(self.start) or self.collision(self.goal):
            # raise Exception('Start or goal are inside the obstacle space, CANBIAR EN rrtplannerPC.py, init')
     
    def goal_colision(self):
        # if self.collision(self.start) or self.collision(self.goal):
        if self.collision(self.goal): #MODIFICADO POR MI, no importa si el start esta en un obstaculo
            # raise Exception('Start or goal are inside the obstacle space, CANBIAR EN rrtplannerPC.py, init')
            print('Goal are inside the obstacle space')
    ...
    def extend(self, tree, qrand):
        ...
        if not self.collision(qnew):
            ...
            # Estaba quitado:
            # reached_new_config = self.distance(qnew, qrand) < 1.5 #MIO
            # if goal_reached:
    ...
    def build_inner_circles(self, inner_radius, outer_radius, z):
    ...
    def plot(self):
        # plot obstacles
        plt.scatter(self.pc_traversable[:, 0], self.pc_traversable[:, 1], color='mediumblue', label = 'Traversable', s=8)
        plt.scatter(self.pc_obstacles[:, 0], self.pc_obstacles[:, 1], color='black', label = 'Obstacles', s=8)  #s = 10 añadido por mi
        plt.scatter(self.start[0], self.start[1], color='orange', label = 'Start', zorder=10)
        plt.scatter(self.goal[0], self.goal[1], color='red', label = 'Goal', zorder=10)
        # if show:
        #     plt.show()

    def plot_point(self, point):
        # plot obstacles
        plt.scatter(self.pc_obstacles[:, 0], self.pc_obstacles[:, 1], color='black', label = 'Obstacles')
        plt.scatter(self.pc_traversable[:, 0], self.pc_traversable[:, 1], color='mediumblue', label = 'Traversable')
